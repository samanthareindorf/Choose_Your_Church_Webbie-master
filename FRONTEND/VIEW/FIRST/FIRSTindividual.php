...
// testimony form controller--some changes need to be here. Should be member and not fellowship, 
            ...
            if(isset($_POST['postFIRST'])){
                ...
                $sqlQuery="INSERT INTO first_user_testimony (user_id,testimony_statement, testimony_date) VALUES ('$user_id','$testimony_statement','$testimony_date')";
...
<head>
    ...
    <title>FIRST LOVE member  Page Personal</title>
...
<form style="margin-left: 270px; margin-top: -779px; width:1100px" method="POST">
  ...
  <button type="submit" name="postFIRST" class="btn btn-primary">Submit</button>
</form><br>
...
<div class="card w-75" style="margin-top: 12px; margin-left:270px">
  ...
  <div class="card-body">
      <!--get testimonies of the user-->
     
      <?php 

                            $user_id=$_SESSION['auth_user']['user_id'];
                            $query="SELECT users.user_id,users.fname, users.lname, first_user_testimony.testimony_statement, first_user_testimony.testimony_id, first_user_testimony.testimony_date FROM users 
                            INNER JOIN first_user_testimony on first_user_testimony.user_id=$user_id WHERE fellowship_id=114 AND users.user_id=$user_id";
